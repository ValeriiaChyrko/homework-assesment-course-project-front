...
export async function PATCH(_req: Request, { params }: { params: Promise<{ courseId: string }> }) {
    try {
        ...
        if (!courseId) {
            console.warn("[COURSE] UNPUBLISH: Missing courseId in params");
        ...
        const { data, status } = await fetchWithAuth({
            method: "PATCH",
            token,
            url: `${process.env.NEXT_PUBLIC_API_URL}/api/courses/${courseId}/unpublish`,
        });

        return NextResponse.json({ assignment: data }, { status });
    } catch (e) {
        console.error("[COURSE_UNPUBLISH]", e);
